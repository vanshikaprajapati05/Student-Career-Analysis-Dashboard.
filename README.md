# 🎯 Student Career & Skill Gap Analytics Dashboard

## Overview

The Student Career & Skill Gap Analytics Dashboard is a Power BI project designed to analyze student performance, technical skills, career preferences, placement readiness, and professional development activities. The dashboard provides actionable insights into skill gaps and employability factors through interactive visualizations and KPI-driven analysis.

## Objectives

* Evaluate student skill proficiency across key technical and professional domains.
* Identify skill gaps and areas for improvement.
* Analyze career preferences and expected salary trends.
* Assess placement readiness using academic and skill-based indicators.
* Examine the impact of internships, projects, and certifications on employability.

## Dataset

The dataset contains 500 student records with the following attributes:

* Student ID
* Gender
* Age
* CGPA
* Python Score
* SQL Score
* Power BI Score
* Excel Score
* Machine Learning Score
* Communication Score
* Projects
* Internships
* Certifications
* Career Path
* Expected Salary

## Dashboard Structure

### 1. Overview Dashboard

Provides a high-level summary of student performance, skill levels, salary expectations, and career distribution.
![Alt Text](https://github.com/vanshikaprajapati05/Student-Career-Analysis-Dashboard./blob/d41e03313d7ca73aa6fd6df580d43f594c140c4e/Screenshots/Screenshot%202026-06-06%20231920.png)

### 2. Skill Gap Analysis

Identifies strengths, weaknesses, and opportunities for skill development across various competencies.
![Alt Text](https://github.com/vanshikaprajapati05/Student-Career-Analysis-Dashboard./blob/fd29cdc6ed45a31fa4ae3ac8bf735eb9181971ee/Screenshots/Screenshot%202026-06-06%20231945.png)

### 3. Career Path Insights

Explores career preferences, skill profiles, and expected salary trends across different career paths.
![Alt Text](https://github.com/vanshikaprajapati05/Student-Career-Analysis-Dashboard./blob/c70b9faf34dc5bb4e85cabe1a80a24de88786ed7/Screenshots/Screenshot%202026-06-06%20232007.png)

### 4. Placement Readiness Analysis

Measures employability readiness through readiness scores and placement status indicators.
![Alt Text](https://github.com/vanshikaprajapati05/Student-Career-Analysis-Dashboard./blob/79fd539bc14dcb4b8aac0e09bae551e63f9de4a2/Screenshots/Screenshot%202026-06-06%20232040.png)

### 5. Internship & Experience Insights

Analyzes the relationship between internships, projects, certifications, and career readiness.

## Key Features

* Interactive KPI Cards
* Dynamic Slicers and Filters
* Career Path Analysis
* Skill Gap Assessment
* Placement Readiness Evaluation
* Internship & Certification Insights
* Multi-Page Dashboard Design

## Tools & Technologies

* Power BI
* DAX (Data Analysis Expressions)
* Power Query
* Data Modeling
* Data Visualization

## Key Insights

* Career paths exhibit varying skill requirements and salary expectations.
* Skill proficiency significantly influences placement readiness.
* Internships and certifications contribute positively to employability indicators.
* Student readiness levels differ across career domains and experience profiles.


## Author

Vanshika Prajapati
B.Sc. Computer Science Student

